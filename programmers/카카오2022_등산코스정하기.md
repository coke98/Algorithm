...
def solution(n, paths, gates, summits):
    ...
    while pq:
        intensity, min_index = heapq.heappop(pq)

        if intensity > intensities[min_index]: continue

        if min_index in summits_set:
            if min_intensity >= intensity:
                min_intensity = intensity
                if min_top > min_index:
                    min_top = min_index
            continue

        for w, target in graph[min_index]:
            if target in gates_set: continue
            new_intensity = max(w, intensity)
            if new_intensity < intensities[target]:
                intensities[target] = new_intensity
                heapq.heappush(pq, (new_intensity, target))
...
- 주어진 파라미터를 set으로 변환해서 사용함.
  - 그냥 사용하면 in 연산에서 너무 많은 연산이 걸림